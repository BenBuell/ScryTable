...
When the shell or caching strategy changes materially, update `CACHE_NAME` in `sw.js` (for example, `scrytable-shell-v2`) and deploy. The service worker removes older `scrytable-*` caches on activation. Users may need to fully close and reopen the app after a major update; the app does not aggressively reload during a game.