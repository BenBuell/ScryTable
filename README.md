...
When the shell or caching strategy changes materially, update `CACHE_NAME` in `sw.js` (currently `scrytable-shell-v12`) and deploy. The service worker removes older `scrytable-*` caches on activation. Users may need to fully close and reopen the app after a major update; the app does not aggressively reload during a game.

## Testing Firebase Realtime Database Rules

Run `pnpm test:firebase-rules` to start the local Realtime Database Emulator and check turn-state authorization. The test suite uses the demo project `demo-scrytable`, does not access production data, and requires Java 21. The emulator downloads its database JAR on first run.