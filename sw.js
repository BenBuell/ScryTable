const CACHE_NAME = "scrytable-shell-v13";
const APP_SHELL = [
  "/",
  "/index.html",
  "/pwa.js",
...
self.addEventListener("fetch", (event) => {
  ...
  event.respondWith(
    caches.match(request, { ignoreSearch: true }).then((cached) => {
      ...
      // Keep the viewport helper in sync with freshly served HTML instead of
      // running the previous cached helper for the first launch after updates.
      return (isNavigation || url.pathname === "/pwa.js") ? networkFetch : (cached || networkFetch);
    })
  );
});
